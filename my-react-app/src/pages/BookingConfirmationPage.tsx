...
export default function BookingConfirmationPage() {
  ...
  const totalGuests = booking?.rooms?.reduce((s, r) => s + r.max_adults + r.max_children, 0) || 0
      ...
      <div className="max-w-[1100px] mx-auto px-4 sm:px-6 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-8 items-start">
          ...
          <div className="order-2 lg:order-1 space-y-5">
                ...
                {booking.rooms.map((r, i) => (
                  <div key={i} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="ml-auto text-right shrink-0">
                      <p className="text-sm font-bold text-gray-900">₹{r.subtotal.toFixed(2)}</p>
            ...
            {/* Guest details */}
            {(stateData?.guestFirstName || stateData?.guestEmail) && (
              <div className="bg-white rounded-xl border border-gray-200 p-5">
                <h2 className="text-base font-bold text-gray-900 mb-3">Guest details</h2>
                <div className="space-y-2 text-sm">
                  {stateData.guestFirstName && (
                    <div className="flex gap-2">
                      <span className="text-gray-500">Name:</span>
                      <span className="text-gray-900 font-medium">{stateData.guestFirstName} {stateData.guestLastName}</span>
                    </div>
                  )}
                ...
                {booking.coupon_code && booking.coupon_discount > 0 && (
                  <div className="mb-3 rounded-lg bg-green-50 border border-green-200 px-3 py-2">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-bold text-green-700">-₹{booking.coupon_discount.toFixed(2)}</span>
                    </div>
                ...
                {booking.special_offer_discount > 0 && (
                  <div className="mb-3 rounded-lg bg-green-50 border border-green-200 px-3 py-2">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-bold text-green-700">-₹{booking.special_offer_discount.toFixed(2)}</span>
                    </div>
                  </div>
                )}
                <div className="space-y-2 text-sm">
